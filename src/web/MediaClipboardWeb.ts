 ...
 */
export class MediaClipboardWeb implements MediaClipboardInterface {
  /**
   ...
   */
  private isAdvancedClipboardApiAvailable(): boolean {
    return (
  ...
  async copyImage(
    ...
    if (!this.isAdvancedClipboardApiAvailable()) {
      throw new Error(
        'Image copying is not supported in this browser. Use a modern browser with Clipboard API support.',
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch image from URL: ${resolvedPath} (${response.status}: ${response.statusText})`,
        );
      }
      ...
      if (!blob.type.startsWith('image/')) {
        throw new Error(
          `File is not a valid image at URL: ${resolvedPath} (received ${blob.type})`,
        );
      }
  ...
  async copyVideo(
    ...
    try {
      ...
      throw new Error(
        `Video copying is not supported in web browsers. Video path resolved to: ${resolvedPath}. Consider copying the video URL instead.`,
      );
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
  ...
  async copyPDF(pdfPath: string, _options?: CopyFileOptions): Promise<void> {
    try {
      // Resolve path to validate it exists (even though we can't copy PDFs)
      const resolvedPath = this.resolveAssetPath(pdfPath);
      throw new Error(
        `PDF copying is not supported in web browsers. PDF path resolved to: ${resolvedPath}. Consider copying the PDF URL instead.`,
      );
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      ...
      if (errorMessage.includes('RELATIVE_PATH_ERROR')) {
        throw new Error(
          `Cannot copy from relative path '${pdfPath}'. Relative paths like './' and '../' are not supported. Please use one of these alternatives:\n1. Use HTTP/HTTPS URL: https://example.com/document.pdf\n2. Use absolute path: /absolute/path/to/document.pdf\n3. Use blob URL: blob:https://example.com/uuid`,
        );
      }
  ...
  async copyAudio(
    ...
    try {
      ...
      throw new Error(
        `Audio copying is not supported in web browsers. Audio path resolved to: ${resolvedPath}. Consider copying the audio URL instead.`,
      );
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      ...
      if (errorMessage.includes('RELATIVE_PATH_ERROR')) {
        throw new Error(
          `Cannot copy from relative path '${audioPath}'. Relative paths like './' and '../' are not supported. Please use one of these alternatives:\n1. Use HTTP/HTTPS URL: https://example.com/audio.mp3\n2. Use absolute path: /absolute/path/to/audio.mp3\n3. Use blob URL: blob:https://example.com/uuid`,
        );
      }
  ...
  async copyFile(
    ...
    try {
      ...
      if (!this.isAdvancedClipboardApiAvailable()) {
        throw new Error(
          `File copying is not supported in this browser. Use a modern browser with Clipboard API support. File path resolved to: ${resolvedPath}`,
        );
      }
      ...
      if (mimeType === 'text/plain' || mimeType === 'text/html') {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to fetch file from URL: ${resolvedPath} (${response.status}: ${response.statusText})`,
            );
          }
          ...
          const errorMessage =
            error instanceof Error ? error.message : String(error);
          throw new Error(
            `Cannot copy text file from resolved path: ${resolvedPath} - ${errorMessage}`,
          );
        }
      }
      ...
      if (mimeType.startsWith('video/')) {
        throw new Error(
          `Video files (${mimeType}) are not supported for copying in web browsers. File path resolved to: ${resolvedPath}. Consider copying the file URL instead.`,
        );
      }

      if (mimeType === 'application/pdf') {
        throw new Error(
          `PDF files are not supported for copying in web browsers. File path resolved to: ${resolvedPath}. Consider copying the file URL instead.`,
        );
      }

      if (mimeType.startsWith('audio/')) {
        throw new Error(
          `Audio files (${mimeType}) are not supported for copying in web browsers. File path resolved to: ${resolvedPath}. Consider copying the file URL instead.`,
        );
      }

      throw new Error(
        `MIME type '${mimeType}' is not supported for copying in web browsers. File path resolved to: ${resolvedPath}. Supported types: images, text files.`,
      );
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
   ...
   */
  private fallbackCopyText(text: string): Promise<void> {
