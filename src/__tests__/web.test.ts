...
class MockClipboardItem {
  ...
  getType = jest.fn((type: string) => Promise.resolve(this.#data[type]));
  static supports = jest.fn(() => true);
}
global.ClipboardItem = MockClipboardItem as any;

describe('MediaClipboardWeb', () => {
  let clipboardWeb: MediaClipboardWeb;
  ...
  describe('copyText', () => {
    ...
    it('should throw error when legacy method fails', async () => {
      ...
      await expect(clipboardWeb.copyText('Hello World')).rejects.toThrow(
        'Failed to copy text: DOM error',
      );
    });
  });
  ...
  describe('copyImage', () => {
    ...
    it('should throw error when advanced Clipboard API is not available', async () => {
      ...
      await expect(clipboardWeb.copyImage('test.jpg')).rejects.toThrow(
        'Image copying is not supported in this browser',
      );
    });

    it('should throw error for relative paths', async () => {
      await expect(
        clipboardWeb.copyImage('./relative/path.jpg'),
      ).rejects.toThrow('Cannot copy from relative path');
    });
    ...
    it('should throw error when fetch fails', async () => {
      ...
      await expect(
        clipboardWeb.copyImage('https://example.com/missing.jpg'),
    ...
    it('should throw error for non-image content', async () => {
      ...
      await expect(
        clipboardWeb.copyImage('https://example.com/text.txt'),
  ...
  describe('copyVideo', () => {
    it('should throw error for video copying with resolved path info', async () => {
      await expect(
        clipboardWeb.copyVideo('https://example.com/video.mp4'),
      ).rejects.toThrow('Video copying is not supported in web browsers');
    });

    it('should throw error for relative paths', async () => {
      await expect(clipboardWeb.copyVideo('./video.mp4')).rejects.toThrow(
        'Cannot copy from relative path',
      );
    });
  });
  ...
  describe('copyPDF', () => {
    it('should throw error for PDF copying with resolved path info', async () => {
      await expect(
        clipboardWeb.copyPDF('https://example.com/document.pdf'),
      ).rejects.toThrow('PDF copying is not supported in web browsers');
    });

    it('should throw error for relative paths', async () => {
      await expect(clipboardWeb.copyPDF('./document.pdf')).rejects.toThrow(
        'Cannot copy from relative path',
      );
    });
  });
  ...
  describe('copyAudio', () => {
    it('should throw error for audio copying with resolved path info', async () => {
      await expect(
        clipboardWeb.copyAudio('https://example.com/audio.mp3'),
      ).rejects.toThrow('Audio copying is not supported in web browsers');
    });

    it('should throw error for relative paths', async () => {
      await expect(clipboardWeb.copyAudio('./audio.mp3')).rejects.toThrow(
        'Cannot copy from relative path',
      );
    });
  });
  ...
  describe('copyFile', () => {
    ...
    it('should throw error for unsupported video MIME types', async () => {
      await expect(
        clipboardWeb.copyFile('https://example.com/video.mp4', 'video/mp4'),
      ).rejects.toThrow('Video files (video/mp4) are not supported');
    });

    it('should throw error for unsupported PDF MIME types', async () => {
      await expect(
        clipboardWeb.copyFile('https://example.com/doc.pdf', 'application/pdf'),
      ).rejects.toThrow('PDF files are not supported');
    });

    it('should throw error for unsupported audio MIME types', async () => {
      await expect(
        clipboardWeb.copyFile('https://example.com/audio.mp3', 'audio/mp3'),
      ).rejects.toThrow('Audio files (audio/mp3) are not supported');
    });

    it('should throw error for unsupported MIME types', async () => {
      await expect(
        clipboardWeb.copyFile(
    ...
    it('should throw error when advanced Clipboard API is not available', async () => {
      ...
      ).rejects.toThrow('File copying is not supported in this browser');
    });

    it('should throw error for relative paths', async () => {
      await expect(
        clipboardWeb.copyFile('./file.txt', 'text/plain'),
      ).rejects.toThrow('Cannot copy from relative path');
    });
  });
  ...
  describe('copyLargeFile', () => {
    ...
    it('should work without progress callback', async () => {
      ...
      await expect(
        clipboardWeb.copyLargeFile(textUrl, 'text/plain'),
