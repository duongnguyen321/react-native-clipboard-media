...
export class MediaClipboardWeb {
    ...
    async requestPermissions() {
        ...
        try {
            ...
            // Permissions API might not be available or permission name not recognized
            // Try a simple write test instead
            try {
    ...
    async checkPermissions() {
        if (!this.isClipboardApiAvailable()) {
            return {
                granted: false,
                status: 'Clipboard API not available in this browser',
            };
        }
        if (!this.isSecureContext()) {
            return {
                granted: false,
                status: 'Requires secure context (HTTPS)',
            };
        }
        try {
            const permissionStatus = await navigator.permissions.query({
            ...
            return {
                ...
                status: `Clipboard permission: ${permissionStatus.state}`,
            };
        }
        catch (error) {
            ...
            try {
                ...
                return {
                    granted: true,
                    status: 'Clipboard access available (tested)',
                };
            }
            catch (writeError) {
                return {
                    granted: false,
                    status: 'Clipboard access denied or unavailable',
                };
            }
        }
    }
