...
describe('MediaClipboard Integration Tests', () => {
    ...
    describe('Real Data Tests', () => {
        it('should copy real text content from file', async () => {
            ...
            expect(NativeModules.MediaClipboard.copyText).toHaveBeenCalledWith(textContent);
            expect(textContent).toContain('MediaClipboard library');
        });
        it('should copy URL content', async () => {
            const testUrl = 'https://github.com/duonguyen321/react-native-clipboard-media';
            await MediaClipboard.copyText(testUrl);
            const { NativeModules } = require('react-native');
            expect(NativeModules.MediaClipboard.copyText).toHaveBeenCalledWith(testUrl);
        });
        ...
        it('should copy JSON file as generic file', async () => {
            ...
            const { NativeModules } = require('react-native');
            expect(NativeModules.MediaClipboard.copyFile).toHaveBeenCalledWith(jsonFilePath, mimeType, options);
        });
        it('should handle large file copying with progress callback', async () => {
            ...
            const { NativeModules } = require('react-native');
            expect(NativeModules.MediaClipboard.copyLargeFile).toHaveBeenCalledWith(filePath, mimeType, progressCallback, {});
        });
